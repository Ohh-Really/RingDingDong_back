...
public class GoogleOAuth2Service {
    ...
    @Autowired
    public GoogleOAuth2Service(UserRepository userRepository, RestTemplate restTemplate, ModelMapper modelMapper, JwtConfig jwtConfig) {
    ...
    public ResponseEntity<LoginFormDto> startWithGoogle(String code) {
        // 헤더 설정
        ...
        // 유저 정보가 이미 존재하면 정보 반환
        if (userRepository.existsByEmail(googleUserInfo.getEmail())) {
            return ResponseEntity.ok(LoginFormDto.builder().id(googleUserInfo.getId()).email(googleUserInfo.getEmail()).build());
        }
        ...
        // 유저 토큰 반환
        return ResponseEntity.created(null).body(LoginFormDto.builder().id(googleUserInfo.getId()).email(googleUserInfo.getEmail()).build());
    }
